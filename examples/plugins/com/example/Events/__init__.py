...
class Events(pyTensible.Plugin):
	...
	def load(self):
		...
		Resources = {'event_manager': event_manager, 'Event': REvent, 'trigger_event': event_manager.trigger_event}
		...
		if event.name in self.handlers.keys():
			for handler in self.handlers[event.name]:
				try:
					...
					pyTensible.plugin_loader.logger.error("Uncaught exception occurred event handler while triggering event: %s with args: %s", str(event.name), str(event.args) + str(event.kwargs))
